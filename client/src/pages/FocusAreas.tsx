...
export default function FocusAreas() {
  return <PublicPageShell><main className="public-page focus-page">
    ...
    <section className="focus-areas-list section-cream"><div className="page-width">{areas.map(({ id, number, icon: Icon, title, text, accent }) => <article id={id} className={`focus-area-row focus-${accent}`} key={id}><div className="focus-area-index">{number}</div><div className="focus-area-icon"><Icon size={32} strokeWidth={1.5} /></div><div><h3>{title}</h3><p>{text}</p></div><a href="/programs" aria-label={`Explore ${title} programs`}><ArrowUpRight size={22} /></a></article>)}</div></section>
    <section className="focus-bridge section-blue"><div className="page-width"><div><p className="reference-eyebrow light"><span /> Designed to connect</p><h2>Start with what matters <span>to you now.</span></h2></div><p>There is no single route into growth. Choose the skill that feels closest to your next step, then find the people and practice that can help you carry it forward.</p><a className="reference-button white-button" href="/programs">Explore programs <ArrowUpRight size={18} /></a></div></section>
