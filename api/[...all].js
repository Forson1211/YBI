      ...
      for (const post of memoryStore.blogPosts) {
        try {
          await supabase.from("blogPosts").insert({
            ...
            createdAt: post.createdAt instanceof Date ? post.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
      ...
      for (const reg of memoryStore.eventRegistrations) {
        try {
          await supabase.from("eventRegistrations").insert({
            ...
            confirmedAt: reg.confirmedAt instanceof Date ? reg.confirmedAt.toISOString() : reg.confirmedAt || (/* @__PURE__ */ new Date()).toISOString(),
            createdAt: reg.createdAt instanceof Date ? reg.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          });
        } catch {
        }
      ...
      for (const don of memoryStore.donations) {
        try {
          await supabase.from("donations").insert({
            ...
            createdAt: don.createdAt instanceof Date ? don.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          });
        } catch {
        }
      ...
      for (const inq of memoryStore.communityInquiries) {
        try {
          await supabase.from("communityInquiries").insert({
            ...
            createdAt: inq.createdAt instanceof Date ? inq.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          });
        } catch {
        }
      ...
      for (const met of memoryStore.impactMetrics) {
        try {
          await supabase.from("impactMetrics").insert({
            ...
            period: met.period,
            status: met.status,
            createdAt: met.createdAt instanceof Date ? met.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          });
        } catch {
        }
      ...
      for (const opp of memoryStore.opportunities) {
        try {
          await supabase.from("opportunities").insert({
            ...
            createdAt: opp.createdAt instanceof Date ? opp.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          });
        } catch {
        }
      ...
      for (const ses of memoryStore.programSessions) {
        try {
          await supabase.from("programSessions").insert({
            ...
            status: ses.status,
            createdAt: ses.createdAt instanceof Date ? ses.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
            updatedAt: (/* @__PURE__ */ new Date()).toISOString()
          });
        } catch {
        }
...
function getSupabase() {
  ...
  if (url && key) {
    try {
      ...
      if (!_seedPromise) {
        _seedPromise = ensureDatabaseSeeded().catch(
          (err) => console.warn("[Database Auto-Seed] Startup error:", err)
        );
      }
    } catch (e) {
...
function registerStorageProxy(app2) {
  ...
  app2.get("/manus-storage/*", handler3);
  app2.get("/api/manus-storage/*", handler3);
  app2.get("/uploads/*", handler3);
  app2.get("/api/uploads/*", handler3);
}
...
var siteImageUploadInput = z8.object({
  slotKey: z8.string().trim().min(2).max(100),
  fileName: z8.string().trim().min(1).max(220),
  mimeType: z8.enum(["image/jpeg", "image/png", "image/webp", "image/svg+xml"]),
  base64: z8.string().min(10),
  altText: z8.string().trim().max(300).optional()
});
