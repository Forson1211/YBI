...
export default function Programs() {
  ...
  const dynamicPrograms = [
    { id: "public-speaking", image: publicSpeaking.src, alt: publicSpeaking.alt, kicker: "Voice · Presence · Courage", title: "Public Speaking", text: "Practice clear communication, confident delivery, active listening, and the courage to bring an important idea into the room.", icon: Mic2 },
    { id: "entrepreneurship", image: entrepreneurship.src, alt: entrepreneurship.alt, kicker: "Ideas · Enterprise · Responsibility", title: "Entrepreneurship", text: "Move from a meaningful idea to a practical beginning through problem-solving, testing, and shared learning.", icon: Sparkles },
    { id: "generations", image: community.src, alt: community.alt, kicker: "Mentorship · Exchange · Belonging", title: "Generations in Conversation", text: "Create connection between lived experience and fresh perspective so each generation can strengthen the other.", icon: UsersRound },
  ];
  ...
  return <PublicPageShell><main className="public-page programs-page">
    ...
    <section className="programs-list section-cream"><div className="page-width">{dynamicPrograms.map(({ id, image, alt, kicker, title, text, icon: Icon }, index) => <article id={id} className={`program-feature program-feature-${index + 1}`} key={id}><img src={image} alt={alt} /><div className="program-feature-copy"><p>{kicker}</p><div className="program-title-row"><Icon size={30} strokeWidth={1.5} /><h2>{title}</h2></div><span>{text}</span><Link className="reference-text-link" href="/contact">Ask about this program <ArrowRight size={18} /></Link></div></article>)}</div></section>
