...
export default function Team() {
  ...
  const dynamicRoles = [
    { image: teamCommunity.src, alt: teamCommunity.alt, label: "Community hosts", title: "Create a room where people belong.", text: "They make gatherings feel open, respectful, and ready for meaningful conversation.", icon: UsersRound },
    { image: teamPublicSpeaking.src, alt: teamPublicSpeaking.alt, label: "Program builders", title: "Turn practice into confidence.", text: "They shape opportunities that make leadership, learning, and communication tangible.", icon: HandHeart },
    { image: teamEntrepreneurship.src, alt: teamEntrepreneurship.alt, label: "Partners and mentors", title: "Connect experience with possibility.", text: "They contribute insight, encouragement, space, and pathways for participants to grow.", icon: BriefcaseBusiness },
  ];
  ...
  return <PublicPageShell><main className="public-page team-page">
    ...
    <section className="team-roles section-cream"><div className="page-width"><div className="team-role-grid">{dynamicRoles.map(({ image, alt, label, title, text, icon: Icon }, index) => <article className={`team-role-card team-role-${index + 1}`} key={label}><img src={image} alt={alt} /><div><span className="team-role-count">0{index + 1}</span><Icon size={27} strokeWidth={1.6} /><p>{label}</p><h3>{title}</h3><span>{text}</span></div></article>)}</div></div></section>
